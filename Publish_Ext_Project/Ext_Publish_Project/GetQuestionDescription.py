...
class GetQuestionDescription:
    ...
    def scrape(self):
        """Scrapes LeetCode data of questions body.
        """
    ...
    def _get_question(self):
        ...
        for url in enumerate(tqdm(url_list)):
            ...
            # finding the div tag which is containg main material of the Question
            question = soup.find(
                'div', class_='flex h-full w-full flex-1 flex-col')

            # if the question is not found because of the high traffic or loading issue
            if question is None:        
                continue
            else:
        ...
        try:
            if connection.is_connected():
                ...
                if question not in [None, []]:
                    ...
                    # Preparing a list of values to be inserted into the database.
                    ...
                    connection.commit()
                    # sleep for 2 seconds 
                    # once the data is inserted so that the database is not overloaded 
