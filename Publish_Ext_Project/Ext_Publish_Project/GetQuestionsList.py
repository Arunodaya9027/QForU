...
class GetQuestionsList:
    ...
    # Specify the path to the txt file which contains the database credentials
    txt_path = "_db_config.txt"

    # Read the database credentials from the txt file
    ...
    def to_sql(self, db_config) -> None:
        ...
        insert_query = """
            INSERT INTO question_master_coding
            (Session, Topic, Title, QuestionLevel, OriginalLink, Uploaded_By, Uploaded_On, 
            Uploaded_From, QuestionMark, Status)
            VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
        """
        for _, row in self.questions.iterrows():
            if (self.db_to_data(row['link'], db_config)):
                if (row['categorySlug'] != "Database" and row['categorySlug'] != "Shell" and row['categorySlug'] != ""):
                    difficulty = "Basic" if row['difficulty'] == "Easy" else "Moderate" if row['difficulty'] == "Medium" else "High"
                    data = ('2023-2024', row['topicTags'], row['title'], difficulty, 
                            row['link'], 'Arunodaya.Singh_cs21 | ' + GetQuestionsList.host_name, 
                            datetime.datetime.now().isoformat(' ', 'seconds'), 
                           GetQuestionsList.ip_address, 1, 'PENDING')
