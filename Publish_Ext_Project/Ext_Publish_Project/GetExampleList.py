...
class GetExampleList:
    """A class to scrape the list of questions, their topic tags, and company tags.

    Args:
        limit (int, optional): The maximum number of questions to query for from Leetcode's graphql API. 
                               Defaults to 10,000.
    """

    # Specify the path to the txt file
    txt_path = "_db_config.txt"
    ...
    def _get_example(self):
        ...
        print("Scraping questions example ... ", end="")
        # Create an instance of the class
        questions_detailed = GetExampleList.db_to_data(
            self, GetExampleList.db_config)
        flag = 0
        for l, url in enumerate(tqdm(questions_detailed)):
            if GetExampleList.duplicacy_data(self, GetExampleList.db_config, l+1):
                QID = l + 1
                ...
                if question is None:
                    ...
                    description = description.replace(
                        "<strong>Input</strong>", "<strong>Input:</strong>").replace(
                            "<strong>Input: </strong>", "<strong>Input:</strong>").replace(
                                "<strong>Output</strong>", "<strong>Output:</strong>").replace(
                                    "<strong>Output: </strong>", "<strong>Output:</strong>").replace(
                                        "<strong>Explanation</strong>", "<strong>Explanation:</strong>").replace(
                                            "<strong>Explanation: </strong>", "<strong>Explanation:</strong>").replace(
                        "<strong>Explanation : </strong>", "<strong>Explanation:</strong>").replace(
                            "<strong>Explanation :</strong>", "<strong>Explanation:</strong>").replace(
                                "<b>Input:</b>", "<strong>Input:</strong>").replace(
                                    "<b>Output:</b>", "<strong>Output:</strong>").replace(
                                        "<b>Explanation:</b>", "<strong>Explanation:</strong>").replace(
                                            "<b>Explanation  :</b>", "<strong>Explanation:</strong>").replace(
                                                "<b>Explanation  : </b>", "<strong>Explanation:</strong>")
                    ...
                    if (description.count('<strong>Input:</strong>') == 0):
                        ...
                        for i in range(total_pre):
                            ...
                            # questions_detailed.append(Example)
                            GetExampleList.to_sql(
                                self, GetExampleList.db_config, example)
    ...
    def duplicacy_data(self, db_config, QID):
        ...
        try:
            if connection.is_connected():
                cursor = connection.cursor()
                sql = "SELECT Question_Id FROM question_master_detail_coding WHERE Question_Id = %s"
    ...
    def to_sql(self, db_config, example):
        ...
        try:
            if connection.is_connected():
                ...
                insert_query = """INSERT INTO question_master_detail_coding(
                                Question_Id, ExampleNo, Input, ExpectedOutput, Explanation) 
                                VALUES(%s, %s, %s, %s, %s);"""
                values = [example["QuestionID"], example["ExNo"],
